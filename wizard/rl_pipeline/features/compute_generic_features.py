...
class ComputeGenericFeatures:
    ...
    def _compute_advanced_card_feature(self, card: Card):
        cards_remaining_same_color = [
            c for c in self.remaining_cards if c.color == card.color
        ]
        return {
            ...
            **self._compute_card_statistics_on_subset(
                ...
                cards_subset=cards_remaining_same_color
                + self.remaining_trump_cards
                + self.remaining_special_cards,
            ),
        }

    @staticmethod
    def _compute_card_statistics_on_subset(
        card: Card, suffix: str, cards_subset: List[Card]
    ):
        return {
            f"NUMBER_CARDS_{suffix}": len(cards_subset),
            f"NUMBER_SUPERIOR_CARDS_{suffix}": len(
                [c for c in cards_subset if c > card]
            ),
            f"NUMBER_INFERIOR_CARDS_{suffix}": len(
                [c for c in cards_subset if c > card]
            ),
        }
    ...
    def _compute_card_context_features(self) -> GenericCardsContextFeatures:
        return GenericCardsContextFeatures(
            ...
            NUMBER_CARDS_REMAINING_PER_COLOR={
                color: len(
                    [card for card in self.remaining_cards if card.color == color]
                )
                for color in BASE_COLORS
            },
            NUMBER_CARDS_REMAINING_IN_HAND_PER_COLOR={
                color: len([card for card in self._player.cards if card.color == color])
                for color in BASE_COLORS
            },
        )
    ...
    def remaining_cards_in_other_players_hand(self):
        return [
            card
            for player in self._game.definition.players
