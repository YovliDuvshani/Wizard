from config.common import MAGICIAN_NAME, TRUMP_COLOR, JESTER_NAME, BASE_COLORS
...
class TestCard:
    ...
    def test_gt_operator_returns_correct_result(self):
        ...
        assert Card(number=13, color=BASE_COLORS[1]) > Card(
            number=12, color=BASE_COLORS[1]
        )
    ...
    def test_from_representation_outputs_right_card(self):
        assert Card.from_representation(
            Card(special_card=MAGICIAN_NAME).representation
        ) == Card(special_card=MAGICIAN_NAME)
        assert Card.from_representation(
            Card(number=13, color=BASE_COLORS[1]).representation
        ) == Card(number=13, color=BASE_COLORS[1])
